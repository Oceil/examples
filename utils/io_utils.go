...
func GetPath(path string) (string, error){
	...
	// You can just parse this once and make a map. This is just for showcase
	file, err := os.Open("MANIFEST")
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
	// MANIFEST file is in this format where examples_go is the runnable
	// examples_go/{relative path} {Absolute path}
	// .
	// .
	...
	for scanner.Scan() {
		...
		if pair[0] == "examples_go/" + path {
			return pair[1], nil
		}
	}
	// A similar more generic idea can be generated for all test files too
	if err := scanner.Err(); err != nil {
		log.Fatal(err)
	}
	return "", &Notfound{fmt.Sprintf("Path: %q has no absolute path pair in MANIFEST", path)}
}
func readVisitsFromCSV(inputFile string) ([]Visit, error) {
	inputFile, err := GetPath(inputFile)
	if err != nil {
